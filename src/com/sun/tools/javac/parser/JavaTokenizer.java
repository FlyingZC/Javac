...
public class JavaTokenizer {
    ...
    private void scanLitChar(int pos) {
        ...
        } else if (reader.bp != reader.buflen) { // 处理非转义字符
            reader.putChar(true);
        }
    }
    ...
    /** Read an identifier. 读取一个标识符.scanIdent()方法将所有组成标识符的字符从buf数组中读取出来按顺序存储到sbuf数组中,最后作为参数调用names.fromChars()方法获取NameImpl对象
     */
